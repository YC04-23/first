...
def search():
    # /search?q=xxx
    q = request.args.get("quire")
    # filter_by：直接使用字段的名称
    # filter：使用模型.字段名称
    questions =QuestionModel.query.filter(or_(QuestionModel.title.contains(quire),QuestionModel.content.contains(quire))).order_by(db.text("create_time"))
    return render_template("index.html",questions=questions)